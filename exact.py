...
def intersection(filename, inputdir, outputdir, replibrary, min_read):
    ...
    rep.columns = ['CHR', 'START', 'END', 'AGCT', 'STRAND', 'NAME']
    ...
    for name, group in tqdm_notebook(replib_group, desc='replib'):
        if name[1] == '+':
            ...
            end_group = [pos+1 for pos in list(group['END'])]
            start_group = [pos for pos in list(group['END'])]
    ...
    for i in tqdm_notebook(range(np.shape(df)[0]), desc=readsname):
        ...
        if row['CHR'] + row['INS_STRAND'] in replib_tree:
            finter = replib_tree[row['CHR'] + row['INS_STRAND']][row['POS']-1:row['POS']+2]
            if len(finter) > 0:
                for x in finter:
                    replib_out.write('\t'.join([str(x.data), str(row['NUM_READS'])]) + '\n')
